...
function requiredNotConfirmatedReason(_: string | undefined, { parent }: yup.AnyObject) {
  return !parent.wouldToBeConfirmed;
}
function requiredPlayAnyInstrument(value: string | undefined, { parent }: yup.AnyObject) {
...
export const teamWorkSchema = yup.object().shape({
  /* Ficha */
  recordNumber: yup.string().max(2, 'Máximo 2 caracteres!').required('Campo Obrigatório!'),
  ...
  parishCoordinationIndications: yup.array(yup.string()).min(1, 'Campo Obrigatório!'),
