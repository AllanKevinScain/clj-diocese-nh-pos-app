...
export const CourseClientPage = (props: CourseClientPageInterface) => {
  ...
  const { data, isLoading } = useCreateQuery<InfertypePoslSchema[]>({
  ...
  return (
    <div
      ...
      <CoursesBottomBar courseId={courseId} courseNumber={courseNumber} recordType="POSl" />
      ...
      {!isLoading && (
        <ListRecords
          ...
          registerUrl={`/record/posl/register?courseNumber=${courseNumber}`}
        />
      )}
    </div>
  );
